# 📊 Sistema de Automação de Vendas

Este projeto foi desenvolvido para automatizar o processo de vendas e gestão de produtos das empresas, focado na comercialização de bebidas e laticínios (mussarela).

## 🚀 Sobre o Projeto
O objetivo é facilitar o dia a dia do representante comercial, permitindo o cadastro de pedidos, controle de estoque e organização de clientes de forma rápida e eficiente.

## 🛠️ Tecnologias Utilizadas
* **C**: Utilizado para a lógica de processamento e estruturação de dados.
* **PHP**: Responsável pela integração e possível interface web/backend.
* **Ubuntu Linux**: Sistema operacional utilizado para o desenvolvimento e deploy.
* **Git/GitHub**: Controle de versão e versionamento do código.

## 📂 Como rodar o projeto

1. Clone o repositório:
   ```bash
   git clone [https://github.com/quaresmatiago/sistema-vendas.git](https://github.com/quaresmatiago/sistema-vendas.git)
